...
网页对话再聪明，很多时候也只能「给建议」。而 CLI Agent 可以：
...
- **不可分发 / 不易交付**
  - CLI 脚手架即使发布出去了，用户机器上还得「额外」自己把本地 LLM 服务跑起来（甚至还要装模型 / 依赖）
...
- **入口**仍是本地对话 CLI，你在命令行输入问题；`/help`、`/exit`、`/clear` 这类指令依旧由 CLI 本地处理，不走模型
- **问主播路**从「本地服务」变成「远程服务」：
  1. CLI 将用户的 `question` 请求发到远程 LLM 服务
  2. 远程服务再去调用 `qwen3-max` 模型完成推理生成
  3. 模型返回结果给远程服务
  4. 远程服务将 `answer` 回传给 CLI，CLI 负责展示（可流式）
- **新增 CI/CD 部署链路**（工程化关键）：
  - 本地代码 `git push` → 触发 GitHub Actions → 自动部署 / 更新远程 LLM 服务（确保服务端版本可持续迭代、稳定交付）
...
- **工具调用能力受限：远端不适合直接执行高权限操作**
  ...
  - 结论：远端更适合当「大脑」，不适合当「手脚」

- **强依赖于网络** — 一旦服务端挂了，所有 CLI 用户都不可用
- **「记忆 / 会话」变成后端工程问题**
