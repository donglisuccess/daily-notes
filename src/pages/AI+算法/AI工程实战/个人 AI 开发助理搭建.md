...
### 4. GitHub PR 流程
`GitHub` 分成两条链路：
代码推送：`SSH Push` 创建 `PR`：`GitHub REST API` + `GITHUB_TOKEN`
...
**创建 PR 的 API**
```bash
curl -s -X POST "https://api.github.com/repos/donglisuccess/daily-notes/pulls" \ -H "Accept: application/vnd.github+json" \ -H "Authorization: Bearer $GITHUB_TOKEN" \ -H "X-GitHub-Api-Version: 2022-11-28" \ -d '{ "title": "docs: update Claude Code assistant workflow", "head": "docs/update-claude-workflow", "base": "main", "body": "## Summary\n\n- 更新 Claude Code 工作流规范\n\n## Check\n\n- 已查看 git diff\n- 已 push 到 GitHub" }'
```
